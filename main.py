...
trainingJson = opt.json
X = np.array ( json.load(open(trainingJson)) )
z_dims = opt.tiles
...
#Padding the remainder of the level using floors. The padded region will be removed when genrating the levels
...
#Initializing custom weights on netG and netD
def weights_init(m):
...
one = torch.FloatTensor([1]) #For Real Levels
mone = one * -1 #For Generated Levels
...
for epoch in range(opt.niter):
    ...
    while i < num_batches:#len(dataloader):
        ...
        if gen_iterations < 25 or gen_iterations % 500 == 0:
            ...
            Diters = opt.Diters
            print('NEW DITER AFTER 25', Diters)
        ...
        while j < Diters and i < num_batches:
            ...
            # clamping the weight values between -0.1 and 0.1
            for p in netD.parameters():
                p.data.clamp_(opt.clamp_lower, opt.clamp_upper)

            data = X_train[i*opt.batchSize:(i+1)*opt.batchSize]

            # print('i VALUE BEFORE INCREMENTING', i)
            # print('EPOCH VALUE', epoch)
            # print('J VALUE', j)
            # print('GEN ITERATIONS', gen_iterations)        
            i += 1
        ...
        genLoss.append(errG.data[0])
        disLoss.append(errD_fake.data[0])
...
np.save('genLoss.npy', np.array(genLoss))
np.save('EpochCount.npy', np.array(epochList))

plt.plot(epochList, genLoss)
plt.xlabel('Epochs')
plt.ylabel('Gen Loss')
plt.show()
plt.savefig('EpochVsGen')
